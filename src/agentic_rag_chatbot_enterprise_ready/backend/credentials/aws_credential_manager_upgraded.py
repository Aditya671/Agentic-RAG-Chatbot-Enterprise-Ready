...
class AWSCredentialManager:
    ...
    Args:
        secret_name: Optional environment-variable name or AWS Secrets Manager
            secret name/ARN. Environment variables are checked first.
        region_name: AWS region used for Secrets Manager.
        cache_ttl_seconds: Optional in-process cache TTL. ``0`` disables caching
            and preserves the safest/most predictable default.
        max_attempts: Botocore standard retry attempts for AWS API calls.
        connect_timeout: Secrets Manager connection timeout in seconds.
        read_timeout: Secrets Manager read timeout in seconds.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.cache_ttl_seconds = self._validate_non_negative_int(
            cache_ttl_seconds,
            "cache_ttl_seconds",
        )
        self.max_attempts = self._validate_positive_int(max_attempts, "max_attempts")
        self.connect_timeout = self._validate_positive_int(
            connect_timeout,
            "connect_timeout",
        )
        self.read_timeout = self._validate_positive_int(
            read_timeout,
            "read_timeout",
        )
    ...
    def get_session() -> boto3.session.Session:
        ...
        Boto3's Session constructor itself does not prove that usable
        credentials exist. Calling ``get_credentials`` forces credential
        resolution, including workload identity/instance metadata providers.
        """
        try:
            ...
            if isinstance(binary_value, (bytes, bytearray)):
                try:
                    ...
                    raise AWSSecretError(
                        "AWS SecretBinary is not valid UTF-8 text."
                    ) from exc

            raise AWSSecretError("AWS returned an invalid SecretBinary value.")

        raise AWSSecretError(
            "AWS Secrets Manager returned neither SecretString nor SecretBinary."
        )
    ...
    def _get_secret_from_aws(self, secret_name: str) -> str:
        ...
        try:
            ...
            if code in {"AccessDeniedException", "AccessDenied"}:
                raise AWSSecretError(
                    "Access denied while retrieving the configured AWS secret."
                ) from exc

            raise AWSSecretError(
                f"AWS Secrets Manager failed to retrieve secret "
                f"'{secret_name}' ({code})."
            ) from exc
    ...
    def get_secret(self, secret_name: Optional[str] = None) -> str:
        ...
        Environment lookup has precedence for compatibility with local
        development and container-injected secrets.

        The secret value is never logged by this class.
        """
        ...
        if self.client is None:
            raise AWSSecretError(
                f"Secret '{resolved_name}' not found in environment variables "
