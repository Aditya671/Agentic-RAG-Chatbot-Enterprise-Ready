...
class AzureCredentialManager:
    ...
    def __init__(
        ...
        *,
        ...
        self.environment = (environment or os.getenv("ENVIRONMENT") or "local").strip().lower()
        self.use_cli_for_local = use_cli_for_local
        self.additionally_allowed_tenants = list(additionally_allowed_tenants) if additionally_allowed_tenants is not None else ["*"]
    ...
    def _normalize_vault_url(key_vault_url: Optional[str]) -> Optional[str]:
        ...
        if not value:
            return None
        if not (value.startswith("https://") and value.endswith(".vault.azure.net")):
            raise ValueError(
                "key_vault_url must be an Azure Key Vault HTTPS URL ending in '.vault.azure.net'."
            )
        return value.rstrip("/")
    ...
    def get_credential(
        ...
        *,
        ...
        if env in cls.LOCAL_ENVIRONMENTS and use_cli_for_local:
            return AzureCliCredential(additionally_allowed_tenants=allowed)
        return DefaultAzureCredential()

    def get_secret(self, secret_name: str, *, required: bool = True, cache: bool = True) -> Optional[str]:
        ...
        if self.client is None:
            if required:
                raise SecretNotFoundError(
                    f"Secret '{name}' is not configured in the environment and no Key Vault client is configured."
                )
            return None
        ...
        try:
            ...
            if required:
                raise SecretNotFoundError(f"Required secret '{name}' was not found in Azure Key Vault.") from exc
            return None
        except AzureError as exc:
            logger.error("Azure Key Vault request failed for secret '%s'.", name, exc_info=True)
            raise RuntimeError(f"Failed to retrieve secret '{name}' from Azure Key Vault.") from exc

        value = getattr(secret, "value", None)
        if not value:
            if required:
                raise SecretNotFoundError(f"Secret '{name}' exists but contains no value.")
            return None
    ...
    def _validate_secret_name(secret_name: str) -> str:
        ...
        if not all(ch.isalnum() or ch == "-" for ch in name):
            raise ValueError(
                f"Invalid Key Vault secret name '{name}'. Use only alphanumeric characters and hyphens."
            )
        return name
