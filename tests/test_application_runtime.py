...
async def test_question_journey_normalizes_routes_records_evidence_and_returns_trace() -> None:
    evidence = Evidence(source_id="doc-1", source_type="document", locator="page-2", metadata={"provider": "fixture"})

    def answer(request: ApplicationRequest) -> ApplicationResult:
        assert request.question == "What is the policy?"
        return ApplicationResult(response_text="The policy is documented on page 2.", capability=Capability.QUESTION, metadata={"mode": "grounded"}, evidence=(evidence,))

    runtime = ApplicationRuntime({Capability.QUESTION: answer})
    execution = await runtime.execute(ApplicationRequest(question="  What   is the policy?  ", session_id="session-1", actor_id="actor-1"))
...
async def test_explicit_upload_capability_does_not_require_question() -> None:
    ...
    execution = await runtime.execute(ApplicationRequest(capability=Capability.UPLOAD, payload={"file_id": "file-1"}))
