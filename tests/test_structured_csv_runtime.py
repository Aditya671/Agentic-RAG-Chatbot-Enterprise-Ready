...
def test_build_csv_runtime_delegates_loaded_dataframe_and_prompts(monkeypatch):
    captured = {}
    ...
    monkeypatch.setattr(
        "backend.orchestration.structured_csv_runtime.build_structured_query_engine",
        build_engine,
    )
    ...
    assert set(result.engine_kwargs) == {
        "instruction_str",
        "pandas_prompt",
