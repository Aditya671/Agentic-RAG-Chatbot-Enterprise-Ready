...
async def test_question_surface_preserves_identity_and_evidence():
    runtime = FakeRuntime()
    view = await ApplicationSurface(runtime).question("What is the policy?", session_id="s1", actor_id="a1", conversation_id="c1")
...
async def test_upload_surface_uses_canonical_capability():
    runtime = FakeRuntime()
    await ApplicationSurface(runtime).upload([{"name": "policy.pdf", "content": b"data"}], session_id="s1", actor_id="a1")
...
async def test_status_surface_preserves_task_identity():
    runtime = FakeRuntime()
    view = await ApplicationSurface(runtime).index_status("task-7", session_id="s1", actor_id="a1")
...
async def test_history_surface_projects_messages():
    ...
    assert view.to_dict()["messages"][0]["run_id"] == "r1"
