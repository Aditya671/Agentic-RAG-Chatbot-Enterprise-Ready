...
def test_python_sources_compile() -> None:
    files = list((ROOT / "src").rglob("*.py")) + list((ROOT / "frontend").rglob("*.py")) + [ROOT / "main.py"]
    assert files
    for path in files:
...
def test_no_mutable_constructor_defaults_in_agent() -> None:
    path = ROOT / "src" / "agentic_rag_chatbot_enterprise_ready" / "backend" / "orchestration" / "agentic_ai_system.py"
